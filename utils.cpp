...
char* itoa(int num, char* str, unsigned char radix) {
    snprintf(str, sizeof(str), "%d", num);
}

using namespace rgb_matrix;

// namespace rgc_matrix {
...
//TODO	matrix.swapBuffers(true);
	...
	flashing_cursor(canvas, 0,0,3,5,1);

	//print the day name
	int i = 0;
	while(daynames[dow][i])
	{
		flashing_cursor(canvas, i*4,0,3,5,0);
...
//TODO		matrix.swapBuffers(true);
	...
	if (i*4 < 32){
		flashing_cursor(canvas, i*4,0,3,5,1);
	}
	else {
//		Spark.process();	//Give the background process some lovin'
	...
	flashing_cursor(canvas, 0,8,3,5,0);
...
//TODO		matrix.swapBuffers(true);
	...
	flashing_cursor(canvas, suffixposx + 8,0,3,5,0);

	//print the month name on the bottom row
	i = 0;
	while(monthnames[mont][i])
	{
		flashing_cursor(canvas, i*4,8,3,5,0);
...
//TODO		matrix.swapBuffers(true);
	...
	if (i*4 < 32){
		flashing_cursor(canvas, i*4,8,3,5,2);
	}
	else {
		sleep(1000);
	}
